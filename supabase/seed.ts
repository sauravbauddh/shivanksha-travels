...
async function seed() {
  ...
  const { error: destError } = await supabase.from('destinations').upsert(DESTINATIONS, { onConflict: 'slug' });
  if (destError) console.error('Error seeding destinations:', destError);
  else console.log('Destinations seeded.');

  console.log('Seeding packages...');
  const { error: pkgError } = await supabase.from('packages').upsert(PACKAGES, { onConflict: 'slug' });
  if (pkgError) console.error('Error seeding packages:', pkgError);
  else console.log('Packages seeded.');

  console.log('Seeding testimonials...');
  // Testimonials don't have unique slug here, so we might duplicate if run multiple times without cleanup
  // To avoid duplicates, we'd need a unique key or check existence. For now, simple insert.
  // Actually, let's use insert and ignore checks for demo. 
  // Ideally we should have IDs or Slugs.
